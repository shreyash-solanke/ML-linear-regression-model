...
def linreg_closed_form(train_X, train_Y):
	...
	xdot = numpy.dot(x_transpose, x)
	xinv = numpy.linalg.inv(xdot)
	x2dot = numpy.dot(xinv, x_transpose)



	Theta = numpy.dot(x2dot, train_Y) # TODO: compute the closed form solution here. Note: using numpy.linalg.lstsq(...) is *not* the correct answer
	return Theta
...
def loss(Theta, train_X, train_Y):
	...
	arr_identity = numpy.ones([n_var, 1])
	x = numpy.append(arr_identity, train_X, axis=1)
	predict = numpy.dot(x, Theta)
	se = (predict - train_Y)**2
	rv = numpy.mean(se) # TODO: compute the loss here.
	return rv
...
if __name__ == '__main__':
	...
	print("LOSS: ", loss_var)
	comp = numpy.linalg.lstsq(data_X, data_Y)
	print("comp: ", comp)
